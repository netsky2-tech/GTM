...
class InventarioSalidas extends Model 
{
    ...
    public function obtenerSalida($request)
    {
        ...
        $salida->with(['salidaProductos' => function($query) {
            ...
            $query->with(['salidaProductoBaterias' => function($query2) {
                $query2->with('bateria');
            }]);
        }]);
    ...
    {
        $productos = $this->select(['inventario.productos.id_producto','inventario.productos.codigo_sistema','inventario.productos.nombre_comercial',
        ...
        $productos->leftJoin('inventario.bodegas_productos', 'inventario.bodegas_productos.id_bodega_producto','inventario.salidas_productos.id_bodega_producto');
        ...
        if ((!empty($request->q))) {
            ...
            $productos->where(function($query) use($searchValue) {
                $query->Where('inventario.productos.codigo_barra', 'ILIKE', '%' . $searchValue . '%')->orWhere('inventario.productos.nombre_comercial', 'ILIKE', '%' . $searchValue . '%')
    ...
    {
        return $this->hasMany('App\Models\InventarioSalidaProductos','id_salida')
            ->select(['*', DB::raw("CONCAT(descripcion_producto,' (',codigo_producto,')') AS text")])
            ->with('salidaPresentacion');;
    }
    ...
    {
        return $this->belongsTo('App\Models\InventarioTipoSalida','id_tipo_salida');
    }

    public function salidaBodegaEntrante()
    {
        return $this->belongsTo('App\Models\InventarioBodegas','id_bodega_entrante')->select('id_bodega','descripcion');
    }
}
