...
class InventarioBodegaProductos extends Model {

	protected $table = 'inventario.bodegas_productos';
    ...
    {
        $productos = $this->select(['inventario.v_productos.*','inventario.v_productos.codigo_sistema','inventario.v_productos.nombre_comercial','inventario.bodegas_productos.id_bodega_producto'
            ,DB::raw("CONCAT(inventario.v_productos.nombre_comercial,' (',inventario.v_productos.codigo_sistema,')') AS text"),'inventario.bodegas_productos.cantidad as cantidad_disponible']);
        $productos->leftJoin('inventario.v_productos', 'inventario.bodegas_productos.id_producto','inventario.v_productos.id_producto');
        if ((!empty($request->id_bodega))) {
            ...
            $id_empresa_global = 0;
            $productos->where('id_empresa',array($conf,$id_empresa_global));
            if ((!empty($request->q))) {
                $searchValue = $request->q;
                $productos->where(function($query) use($searchValue) {
                    $query->Where('inventario.v_productos.codigo_sistema', 'ILIKE', '%' . $searchValue . '%')->orWhere('inventario.v_productos.nombre_comercial', 'ILIKE', '%' . $searchValue . '%')
    ...
    {
        $productos = $this->select(['inventario.v_productos_venta.unidad_medida','inventario.v_productos_venta.tasa_impuesto','inventario.v_productos_venta.codigo_barra','inventario.v_productos_venta.descripcion','inventario.bodegas_productos.id_bodega_producto'
            ,DB::raw("CONCAT(inventario.v_productos_venta.descripcion,' (',inventario.v_productos_venta.codigo_barra,')') AS text"),'inventario.bodegas_productos.cantidad as cantidad_disponible','inventario.v_productos_venta.precio_sugerido','inventario.v_productos_venta.costo_promedio']);
...
*/
            ...
            'inventario.v_productos_venta.precio_sugerido', 'inventario.v_productos_venta.precio_distribuidor','inventario.v_productos_venta.costo_promedio','inventario.v_productos_venta.tipo_producto']);
        $productos->leftJoin('inventario.v_productos_venta', 'inventario.bodegas_productos.id_producto','inventario.v_productos_venta.id_producto');
        if ((!empty($request->id_bodega))) {
            $productos->Where('inventario.bodegas_productos.id_bodega', $request->id_bodega);
            $productos->Where('inventario.bodegas_productos.cantidad', '>',0);
...
*/
    ...
    public function productosBodegaGarantia($request)
    {
        $productos = $this->select(['inventario.v_productos_venta.id_producto','inventario.baterias_detalles.bci','inventario.v_productos_venta.codigo_sistema','inventario.v_productos_venta.unidad_medida','inventario.v_productos_venta.tasa_impuesto','inventario.v_productos_venta.codigo_barra','inventario.v_productos_venta.descripcion','inventario.bodegas_productos.id_bodega_producto'
            ,DB::raw("CONCAT(inventario.v_productos_venta.descripcion,' (',inventario.v_productos_venta.codigo_barra,')') AS text"),'inventario.bodegas_productos.cantidad as cantidad_disponible','inventario.v_productos_venta.precio_sugerido','inventario.v_productos_venta.costo_promedio','inventario.v_productos_venta.tipo_producto']);
        $productos->leftJoin('inventario.v_productos_venta', 'inventario.bodegas_productos.id_producto','inventario.v_productos_venta.id_producto');
    ...
    public function productosBodegaConsignacionCliente($request)
    {
        $productos = $this->select(['inventario.v_productos_consignados.id_producto','inventario.v_productos_consignados.codigo_sistema','inventario.v_productos_consignados.unidad_medida','inventario.v_productos_consignados.tasa_impuesto','inventario.v_productos_consignados.codigo_barra','inventario.v_productos_consignados.nombre_comercial','inventario.bodegas_productos.id_bodega_producto'
            ,DB::raw("CONCAT(inventario.v_productos_consignados.nombre_comercial,' (',inventario.v_productos_consignados.codigo_barra,')') AS text"),'v_productos_consignados.cantidad_disponible as cantidad_disponible','inventario.v_productos_consignados.precio_sugerido','inventario.v_productos_consignados.costo_promedio','inventario.v_productos_consignados.tipo_producto']);
    ...
    {
        return $this->belongsTo('App\Models\InventarioProductos','id_producto')->select(['id_producto','codigo_sistema','tipo_producto']);
    }
    ...
    public function presentacion(){
        return $this->belongsTo('App\Models\InventarioPresentacion','id_presentacion','id_presentacion');
    }
}