...
class RRHHGenerarPlanilla extends Model
{
    ...
    public function obtenerNumQuincena($anio,$mes)
    {
        $codigo = $this->select([DB::raw("COALESCE(max(num_quincena),0)+1 as secuencia")]);
        if((!empty($anio))&&(!empty($mes))){
            $codigo->where('anio',$anio);
            $codigo->where('mes',$mes);
        }
        return $codigo->first();
    }
