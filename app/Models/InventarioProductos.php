...
class InventarioProductos extends Model 
{
    ...
    {
        $productos = $this->select(['*','nombre_comercial AS text']);
        $searchValue = $request->q;
        $productos->where('activo', 1);
        $productos->where('nombre_comercial', 'ILIKE', '%' . $searchValue . '%');
        $conf = session()->get('id_empresa');
        $productos->where('id_empresa',$conf);
        $productos->orderBy('nombre_comercial', 'asc');
    ...
    {
        $productos = $this->select(['id_producto','codigo_sistema','nombre_comercial AS text']);
        $searchValue = $request->q;
        $productos->where('activo', 1);
        $productos->whereIn('tipo_producto', array( 1,3));
        $productos->where('nombre_comercial', 'ILIKE', '%' . $searchValue . '%')
        ->Orwhere('codigo_sistema', $searchValue);
        $productos->orderBy('nombre_comercial', 'asc');
        return $productos->limit(10)->get();
    }
    ...
    public function obtenerProductos($request)
    {
        ...
        if (!empty($request->search['field'])) {
            ...
            $productos->with('presentacionesProducto');
            ...
            $productos->whereIn('tipo_producto',array(1,2,4));
    ...
    {
       $productos = $this->select(['*',DB::raw("CONCAT(nombre_comercial,' (',codigo_sistema,')') AS text")]);
