...
class InventarioProductosVistaVenta extends Model
{
    ...
    public function serviciosVenta($request)
    {
        $productos = $this->select([
            ...
            DB::raw("CONCAT(inventario.v_productos_venta.descripcion,' (',inventario.v_productos_venta.codigo_barra,')') AS text"),DB::raw("100 AS cantidad_disponible"),'inventario.v_productos_venta.precio_sugerido', 'inventario.v_productos_venta.precio_distribuidor','inventario.v_productos_venta.costo_estandar AS costo_promedio','inventario.v_productos_venta.tipo_producto']);
        $productos->where('tipo_producto',2);
