...
use App\Models\InventarioConteoFisicoBaterias;
...
use App\Models\InventarioConteoFisico;
...
class InventarioConteoFisicoController extends Controller
{

    public function obtener(Request $request, InventarioConteoFisico $conteos)
    {
    ...
    public function obtenerConteo(Request $request, InventarioConteoFisico $conteo_fisico)
    {
        $rules = [
        ...
        if (!$validator->fails()) {
            ...
            $direccion_empresa = AdmonAjustes::where('id_empresa', '=', $conf)->where('id_ajuste',5)->select('valor')->first();

            $consolidado = InventarioConteoFisicoBaterias::
            select('id_conteo_fisico','id_producto', DB::Raw('count(*) as cantidad_prod'))
                ->with('productoSimple')
                ->where('id_empresa', '=', $conf)->where('id_conteo_fisico',$request->id_conteo_fisico)->orderBy('id_producto', 'desc')
            ...
            if(!empty($conteo_fisico)){
                ...
                if($request->cargar_dependencias){
                    //if($conteo_fisico->tipo_productos==2){
                        $bodegas =InventarioBodegas::where('id_empresa', '=', $conf)->where('activo', 1)->select('id_bodega','id_tipo_bodega','descripcion')->whereIn('id_tipo_bodega',array(1,2,3,5))->get();
                    ...
                    /*}else{
                        $productos = InventarioProductos::select('id_producto','descripcion','codigo_barra',DB::raw("CONCAT(inventario.productos.descripcion,' (',inventario.productos.codigo_barra,')') AS text"))->where('activo', 1)->whereIn('tipo_producto',array(3))->with('unidadMedida')->with('productoDetallesBaterias')->orderby('id_producto')->get();
    ...
    public function nuevo(Request $request){
        ...
        $productos = InventarioProductos::select('id_producto','codigo_barra',DB::raw("CONCAT(inventario.productos.descripcion,' (',inventario.productos.codigo_barra,')') AS text"))
            ->where('id_empresa', '=', $conf)->where('activo', 1)->whereIn('tipo_producto',array(3))
            ->with('unidadMedida')
        ...
        $conteo_fisico = new InventarioConteoFisico();
    ...
    public function nuevoManual(Request $request){
        ...
        $productos_herramientas = InventarioProductos::select('id_producto','id_unidad_medida','condicion','codigo_barra',DB::raw("CONCAT(inventario.productos.descripcion,' (',inventario.productos.codigo_barra,')') AS text"))
            ->where('id_empresa', '=', $conf)->where('activo', 1)->whereIn('tipo_producto',array(1))
            ->with('unidadMedida')
            ->Where('condicion',1)
            ->orderby('id_producto')->get();
            ...
            try{
                ...
                if($request->estado == 0){
                    ...
                    if(!empty($request->id_conteo_fisico_bateria)){
                        InventarioConteoFisicoBaterias::where('id_empresa', '=', $conf)->where('id_conteo_fisico_bateria', $request->id_conteo_fisico_bateria)->delete();
                    }
                    ...
                    if(!empty($request->id_conteo_fisico_bateria)){
                        ...
                        /*$bateria_individual = InventarioConteoFisicoBaterias::find($request->id_conteo_fisico_bateria);
                    ...
                    //$bateriaRegistrada = InventarioEntradaInicialProductos::where('codigo_garantia',$request->codigo_garantiax)->where('id_entrada_inicial',$request->id_entrada_inicial)->first();
                      $bateriaRegistrada = InventarioConteoFisicoBaterias::WhereRaw("upper(codigo_garantia) = upper(?)",[$request->codigo_garantia])->where('id_empresa', '=', $request->id_empresa)->where('id_conteo_fisico',$request->id_conteo_fisico)->first();



                    if(empty($bateriaRegistrada)){
                    $bateria_individual = new InventarioConteoFisicoBaterias();
    ...
    public function registrar(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            try{
                ...
                DB::beginTransaction();
                $entrada = InventarioConteoFisico::find($request->id_conteo_fisico);
                        ...
                        if($bateria['estado'] == 0){///Baterias eliminadas
                            ...
                            if ($bateria['registrada']){///Baterias registradas
                                if(!empty($bateria['id_conteo_fisico_bateria'])){
                                    InventarioEntradaInicialProductos::where('id_empresa', '=', $conf)->where('id_conteo_fisico_bateria', $bateria['id_conteo_fisico_bateria'])->delete();
                                }
                            }
