...
class CuentasXCobrarTiposNotaCreditoController extends Controller
{
    /**
    ...
    public function obtener(Request $request, CuentasXCobrarTiposNotasCredito $tipos_nc)
    {
    ...
    public function obtenerTodos(Request $request, CuentasXCobrarTiposNotasCredito $tipos_nc)
    {
        $tipos_nc = CuentasXCobrarTiposNotasCredito::orderby('id_tipo_nota_credito')->get();
        return response()->json([
            'status' => 'success',
            'result' => $tipos_nc,
            'messages' => null
    ...
    public function obtenerTipoNC(Request $request)
    {
        ...
        if (!$validator->fails()) {
            $tipos_nc = CuentasXCobrarTiposNotasCredito::where('id_tipo_nota_credito',$request->id_tipo_nota_credito)->with('tipoCuentaContable')->get();
            $cuentas_contables = ContabilidadCuentasContablesVista::orderBy('cta_contable')->where('estado',1)->select('id_cuenta_contable','cta_contable','nombre_cuenta','nombre_cuenta_completo')->get();
    ...
    public function registrar(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $tipo_nota_credito = new CuentasXCobrarTiposNotasCredito;
    ...
    public function actualizar(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $tipo_nota_credito = CuentasXCobrarTiposNotasCredito::find($request->id_tipo_nota_credito);
            $tipo_nota_credito->descripcion = $request->descripcion;
    ...
    public function desactivar(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $grupo = CuentasXCobrarTiposNotasCredito::find($request->id_tipo_nota_credito);
            $grupo->estado = 0;
            $grupo->save();
