...
class ContabilidadTiposDocumentosController extends Controller
{
    ...
    public function obtener(Request $request, ContabilidadTiposDocumentos $tipos_documentos)
    {
    ...
    public function obtenerTodos(Request $request, ContabilidadTiposDocumentos $tipos_documentos)
    {
        $tipos_documentos = ContabilidadTiposDocumentos::where('permite_registro_manual',1)->where('estado',1)->orderby('id_tipo_doc')->get();
        return response()->json([
            'status' => 'success',
    ...
    public function obtenerTipoDocumento(Request $request)
    {
        ...
        if (!$validator->fails()) {
            $tipo_documento = ContabilidadTiposDocumentos::find($request->id_tipo_doc);
    ...
    public function registrar(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $tipo_doc = new ContabilidadTiposDocumentos;
            ...
            $tipo_doc->estado = 1;
            $tipo_doc->u_creacion = Auth::user()->usuario;
            $tipo_doc->save();
    ...
    public function actualizar(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $tipo_doc = ContabilidadTiposDocumentos::find($request->id_tipo_doc);
            $tipo_doc->descripcion = $request->descripcion;
            $tipo_doc->permite_registro_manual = $request->permite_registro_manual;
            $tipo_doc->prefijo = $request->prefijo;
            $tipo_doc->save();
    ...
    public function desactivar(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $tipo = ContabilidadTiposDocumentos::find($request->id_tipo_doc);
            $tipo->estado = 0;
            $tipo->save();
