...
class ContabilidadCentroCostoIngresoController extends Controller
{
    /**
     ...
     * @param ContabilidadCentroCostoIngreso $centros
     * @return  json(array)
     * @author omcs
     */

    public function obtener(Request $request, ContabilidadCentroCostoIngreso $centros)
    {
    ...
    public function obtenerTodos(Request $request, ContabilidadCentroCostoIngreso $centros)
    {
        $centros = ContabilidadCentroCostoIngreso::orderby('id_centro')->get();
        return response()->json([
            'status' => 'success',
            'result' => $centros,
            'messages' => null
    ...
    public function obtenerCentro(Request $request)
    {
        ...
        if (!$validator->fails()) {
            $centros = ContabilidadCentroCostoIngreso::find($request->id_centro);
    ...
    public function registrar(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $centros = new ContabilidadCentroCostoIngreso();
            ...
            //$centros->id_centro = ContabilidadCentroCostoIngreso::max('id_centro') +1;
           ...
           // $centros->codigo = $this->zero_fill($centros->tipo_centro). $this->zero_fill($secuencia);
            ...
            $centros->u_creacion = Auth::user()->usuario;
            $centros->estado = 1;
            $centros->save();
    ...
    public function actualizar(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $centro = ContabilidadCentroCostoIngreso::find($request->id_centro);

            $centrox= ContabilidadCentroCostoIngreso::where('id_centro','!=',$request->id_centro)
