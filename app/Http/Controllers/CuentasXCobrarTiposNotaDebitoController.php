...
class CuentasXCobrarTiposNotaDebitoController extends Controller
{
    /**
    ...
    public function obtener(Request $request, CuentasXCobrarTiposNotasDebito $tipos_nd)
    {
    ...
    public function obtenerTodos(Request $request, CuentasXCobrarTiposNotasDebito $tipos_nd)
    {
        $tipos_nd = CuentasXCobrarTiposNotasDebito::orderby('id_tipo_nota_debito')->get();
        return response()->json([
            'status' => 'success',
            'result' => $tipos_nd,
            'messages' => null
    ...
    public function obtenerTipoND(Request $request)
    {
        ...
        if (!$validator->fails()) {
            $tipos_nd = CuentasXCobrarTiposNotasDebito::where('id_tipo_nota_debito',$request->id_tipo_nota_debito)->with('tipoCuentaContable')->get();
            $cuentas_contables = ContabilidadCuentasContablesVista::orderBy('cta_contable')->where('estado',1)->select('id_cuenta_contable','cta_contable','nombre_cuenta','nombre_cuenta_completo')->get();
    ...
    public function registrar(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $tipo_nota_debito = new CuentasXCobrarTiposNotasDebito;
    ...
    public function actualizar(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $tipo_nota_debito = CuentasXCobrarTiposNotasDebito::find($request->id_tipo_nota_debito);
            $tipo_nota_debito->descripcion = $request->descripcion;
    ...
    public function desactivar(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $grupo = CuentasXCobrarTiposNotasDebito::find($request->id_tipo_nota_debito);
            $grupo->estado = 0;
            $grupo->save();
