...
use App\Models\ContabilidadCierresMensuales;
...
class ContabilidadCuentasContablesController extends Controller
{
    ...
    public function buscarCuentasContables(Request $request, ContabilidadCuentasContablesVista $cuentas_contables)
    {
    ...
    public function buscarCuentasContablesF(Request $request, ContabilidadCuentasContablesVista $cuentas_contables)
    {
    ...
    public function obtenerCuentasContables(Request $request, ContabilidadCuentasContablesVista $cuentas_contables)
    {
        //echo gethostname();
    ...
    public function obtenerCuentaContable(Request $request, ContabilidadCuentasContables $cuenta_contable)
    {
        $rules = [
    ...
    public function obtenerCuentaContableV(Request $request, ContabilidadCuentasContablesVista $cuenta_contable)
    {
        $rules = [
            'id_cuenta_contable' => 'required|integer|min:1'
        ];
    ...
    public function obtenerTodasCuentasContables(Request $request, ContabilidadCuentasContablesVista $cuentas_contables)
    {
        $cuentas_contables = ContabilidadCuentasContablesVista::orderBy('cta_contable')->get();
        return response()->json([
            'status' => 'success',
    ...
    public function obtenerCuentasContablesNivel(Request $request, ContabilidadCuentasContablesVista $cuentas_contables)
    {
	...
	{ 
		...
		if (!$validator->fails()) {
			...
			try{
			DB::beginTransaction();
			$cuenta_contable = new ContabilidadCuentasContables;
           ...
           /* if((!empty($request->codigo_cuenta)) && $request->codigo_cuenta > 0 && $request->codigo_cuenta < 10 && ($request->nivel_cuenta['id_nivel_cuenta'] > 2))
			...
			$periodo = ContabilidadPeriodoFiscal::where('estado',0)->first();

			if(!empty($periodo)){
            $cierres = new ContabilidadCierresMensuales();
                $cierres->id_periodo = $periodo->id_periodo;
	...
	public function actualizarCuentaContable(Request $request)
	{
			...
			if (!$validator->fails()) {
				...
				try{
				DB::beginTransaction();
				$cuenta_contable = ContabilidadCuentasContables::findOrFail($request->id_cuenta_contable);
    ...
    public function activarCuentaContable(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $cuenta_contable = ContabilidadCuentasContables::find($request->id_cuenta_contable);
            $cuenta_contable->estado = 1;
            $cuenta_contable->save();
       ...
       // echo $ext;
        ...
        if (in_array($ext, $os)) {
            ...
            */
            ...
            $input = '/var/www/html/resources/reports/CuentasContables';
            $output = '/var/www/html/resources/reports/'.$hora_actual.'CuentasContables';
