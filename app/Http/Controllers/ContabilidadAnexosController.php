...
use Validator,Auth;
use App\Models\ContabilidadAnexos;
use Illuminate\Http\Request;
use Illuminate\Validation\Rule,DB;
class ContabilidadAnexosController extends Controller
{
    /**
    ...
    public function obtenerAnexos(Request $request, ContabilidadAnexos $anexos)
    {
    ...
    public function obtenerTodosAnexos(Request $request, ContabilidadAnexos $anexos)
    {
        $anexos = ContabilidadAnexos::all();
        return response()->json([
            'status' => 'success',
            'result' => $anexos,
            'messages' => null
    ...
    public function obtenerAnexosPorEstadoFinanciero(Request $request, ContabilidadAnexos $anexos)
    {
        $rules = [
        ...
        if (!$validator->fails()) {
            $anexos = ContabilidadAnexos::where('id_estado_financiero',$request->id_estado_financiero)->where('estado',1)->get();
            return response()->json([
                'status' => 'success',
                'result' => $anexos,
                'messages' => null
    ...
    public function obtenerAnexo(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $anexo = ContabilidadAnexos::where('id_anexo',$request->id_anexo)->with('anexosEstadoFinanciero')->get();
            if(!empty($anexo[0])){		
    ...
    {
        $rules = [
            'nombre_anexo' => 'required|string|max:50|unique:pgsql.contabilidad.anexos,nombre_anexo',
            'anexos_estado_financiero' => 'required|array|min:1'
        ];

        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $anexo = new ContabilidadAnexos;
    ...
    public function actualizarOrdenAnexos(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            foreach ($request->anexos as $anexo) {
                $anexoX = ContabilidadAnexos::find($anexo['id_anexo']);
        ...
        $rules = [
           ...
           // 'descripcion' => 'required|string|max:100|unique:pgsql.admon.roles,descripcion',
            'nombre_anexo' => [
                ...
                'string',
                'max:50',
                Rule::unique('pgsql.contabilidad.anexos')->ignore($request->id_anexo,'id_anexo')
            ],
        ];

        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $anexo = ContabilidadAnexos::find($request->id_anexo);
            $anexo->nombre_anexo = $request->nombre_anexo;
            $anexo->save();
