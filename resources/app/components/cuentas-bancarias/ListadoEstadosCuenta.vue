	...
	<div class="main">
		<div class="content-box">
			<div class="content-box-header">
				<div class="box-title">Listado de Estados de cuenta</div>
				<div class="box-description">Estados de cuenta por cuenta bancaria</div>
				<div class="box-description"><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal' }">Módulos</router-link>><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal-caja' }"> Módulo Caja y Bancos</router-link>> Listado Estados de cuenta</div>
			</div>
			<div class="row">
				<div class="col-md-3 sm-text-center">
					<router-link :to="{ name: 'registrar-estado-cuenta' }" class="btn btn-success" tag="button">
						<i class="pe-7s-plus"></i> Registrar Estados de Cuenta
					</router-link>
				</div>
			...
			<div class="table-responsive mt-3">
				<table class="table table-striped table-bordered">
					...
					<tbody>
						<tr :key="solicitud.id_estado_cuenta_banco" v-for="solicitud in estados_cuenta">
							<td>{{ solicitud.periodo_mes_estado_cuenta?solicitud.periodo_mes_estado_cuenta.descripcion:"N/D" }}</td>
							<td>{{ solicitud.cuenta_bancaria_estado_cuenta?solicitud.cuenta_bancaria_estado_cuenta.numero_cuenta:"N/D" }}</td>
							<td class="text-center">{{ solicitud.cantidad_movimientos}}</td>
							<td class="text-center">{{ solicitud.total_debitos |formatMoney(2)}}</td>
							<td class="text-center">{{ solicitud.total_creditos |formatMoney(2)}}</td>
							...
							<td class="text-center">
								<router-link v-if="solicitud.estado === 1"  v-tooltip="'Generar Conciliación'" :to="{ name: 'registrar-conciliacion', params: { id_estado_cuenta_banco: solicitud.id_estado_cuenta_banco } }" tag="a"><i class="icon-pencil"></i></router-link>

							</td>
							<td class="text-center">
								<a v-if="solicitud.estado === 1"  v-tooltip="'Anular Estado de Cuenta'" @click.prevent="anular(solicitud.id_estado_cuenta_banco)">  <i class="fa fa-remove"></i></a>
							</td>
						</tr>
						<tr v-if="!estados_cuenta.length">
							<td class="text-center" colspan="5">Sin datos</td>
						</tr>
					</tbody>
				</table>
			</div>
			<pagination :items="estados_cuenta" :limit="filter.limit" :limitOptions="filter.limitOptions" :page="filter.page" :total="total" @changeLimit="changeLimit" @changePage="changePage"></pagination>
...
<script type="text/ecmascript-6">
	...
	export default {
		...
		methods: {
			...
			descargarReporte(id_estado_cuenta_banco) {
				var self=this;
				var formato='xls';
