	...
	<div class="main">
		<div class="content-box">
			<div class="content-box-header">
				<div class="box-title">Administrar Unidades de medida</div>
				<div class="box-description">Listado de Unidades de medida</div>
				<div class="box-description"><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal' }">Módulos</router-link>><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal-inventario' }"> Módulo inventario</router-link>> Unidad de medida</div>
			</div>
			<div class="row">
				<div class="col-md-4 sm-text-center">
					<router-link class="btn btn-success" tag="button" :to="{ name: 'registrar-unidad-medida' }">
						<i class="pe-7s-plus"></i> Crear Nueva Unidad de medida
					</router-link>
				</div>
				...
				<table class="table table-striped table-bordered">
					<thead>
						<tr>
							...
							<th>Entrante</th>
							<th>Saliente</th>
							<th>Relación con UM estándar</th>
							<th class="text-center table-number">Estado</th>
							<th class="text-center action">Opciones</th>
						</tr>
					</thead>
					<tbody>
						<tr :key="um.id_unidad_medida" v-for="um in unidades_medida">
							<td>{{ um.descripcion}}</td>
							<td>{{ um.unidad_medida }}</td>
							<td>{{um.unidad_medidad.clasificacion_um}}</td>
							<td>
								<div v-if="um.entrante===true">
									<span>Sí</span>
								</div>
								<div v-else>
									<span>No</span>
							...
							<td v-if="um.id_empresa != null" class="text-center">
								<router-link :to="{ name: 'actualizar-unidad-medida', params: { id_unidad_medida: um.id_unidad_medida } }" tag="a"><i class="icon-pencil"></i></router-link>
							</td>
						</tr>
