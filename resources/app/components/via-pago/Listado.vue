	...
	<div class="main">
		<div class="content-box">
			<div class="content-box-header">
				<div class="box-title">Administrar Via Pago</div>
				<div class="box-description">Listado de Vías Pago</div>
				<div class="box-description"><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal' }">Módulos</router-link>><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal-caja' }"> Módulo caja y banco</router-link>> Via pago</div>
			</div>
			<div class="row">
				<div class="col-md-3 sm-text-center">
					<router-link :to="{ name: 'registrar-via-pago' }" class="btn btn-success" tag="button">
						<i class="pe-7s-plus"></i> Registrar Vía Pago
					</router-link>
				</div>
			...
			<div class="table-responsive mt-3">
				<table class="table table-striped table-bordered">
					...
					<tbody>
						<tr :key="via.id_via_pago" v-for="via in vias">
							<!--<td>{{ via.id_via_pago }}</td>-->
							...
							<td class="text-center">
								<router-link :to="{ name: 'actualizar-via-pago', params: { id_via_pago: via.id_via_pago } }" tag="a"><i class="icon-pencil"></i></router-link>
							</td>
						</tr>
						<tr v-if="!vias.length">
			...
			<pagination :items="vias" :limit="filter.limit" :limitOptions="filter.limitOptions" :page="filter.page" :total="total" @changeLimit="changeLimit" @changePage="changePage"></pagination>
		...
		methods: {
			...
			downloadItem (ext) {
				...
				if(!this.descargando){
					...
					axios.get('via-pago/reporte/'+ext, { responseType: 'blob' })
							.then(({ data }) => {
