	...
	<div class="main">
		<div class="content-box">
			<div class="content-box-header">
				<div class="box-title">Administrar configuracion IR</div>
				<div class="box-description">Listado de configuraciones de IR</div>
				<div class="box-description"><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal' }">Módulos</router-link>><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal-nomina' }"> Módulo nomina</router-link>> Configuraciones IR</div>
			</div>
			<div class="row">
				<div class="col-md-6 sm-text-center">
					<router-link :to="{ name: 'registrar-configuracion-ir' }" class="btn btn-success" tag="button">
			...
			<div class="table-responsive mt-3">
				<table class="table table-striped table-bordered">
					...
					<tbody>
						<tr :key="configuracion.id_cat_configuracion_deduccion" v-for="configuracion in configuraciones">
							<td>{{ configuracion.monto_inicial}}</td>
							<td>{{  configuracion.monto_final}}</td>
							<td>{{  configuracion.impuesto_base }}</td>
							<td>{{ configuracion.porcentaje }}</td>
							<td class="text-center">
							 <div v-if="configuracion.estado">
							...
							<td class="text-center">
								<router-link :to="{ name: 'actualizar-configuracion-ir', params: { id_configuracion_ir: configuracion.id_configuracion_ir } }" tag="a"><i class="icon-pencil"></i></router-link>
							</td>
						</tr>
						<tr v-if="!configuraciones.length">
							<td class="text-center" colspan="5">Sin datos</td>
						</tr>
					</tbody>
				</table>
			</div>
			<pagination :items="configuraciones" :limit="filter.limit" :limitOptions="filter.limitOptions" :page="filter.page" :total="total" @changeLimit="changeLimit" @changePage="changePage"></pagination>
	...
	export default {
		...
		methods: {
			...
			obtener() {
				var self = this
				self.loading = true;
				configuracion.obtener(self.filter, data => {
