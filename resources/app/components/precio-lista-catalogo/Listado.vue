	...
	<div class="main">
		<div class="content-box">
			<div class="content-box-header">
				<div class="box-title">Administrar Catalogo de lista de precio</div>
				<div class="box-description">Listado de catalogos</div>
				<div class="box-description"><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal' }">Módulos</router-link>><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal-ventas' }"> Módulo venta</router-link>> Catalogo lista de precio</div>
			</div>
			<div class="row">
				<div class="col-md-6 sm-text-center">
					<router-link :to="{ name: 'registrar-lista-catalogo' }" class="btn btn-success" tag="button">
						<i class="pe-7s-plus"></i> Registrar catalogo
					</router-link>
				</div>
				...
				<table class="table table-striped table-bordered">
					...
					<tbody>
						<tr :key="lista.id_precio_lista_catalogo" v-for="lista in listas">
							...
							<td>{{ lista.valida_desde }}</td>
							<td>{{ lista.valido_hasta }}</td>
							<td class="text-center">
							 <div v-if="lista.estado == 1">
                              <span class="badge badge-success">Activo</span>
                             </div>
								<div v-else-if="lista.estado == 2">
                             		 <span class="badge badge-info">En uso</span>
                                 </div>
								<div v-else-if="lista.estado == 0">
									<span class="badge badge-danger">Desactivado</span>
								</div>
							</td>
							<td class="text-center">
								<router-link :to="{ name: 'actualizar-lista-catalogo', params: { id_precio_lista_catalogo: lista.id_precio_lista_catalogo } }" tag="a"><i class="icon-pencil"></i></router-link>
							</td>
						</tr>
	...
	export default {
		...
		methods: {
			obtener() {
				var self = this
				self.loading = true;
				lista.obtener(self.filter, data => {
