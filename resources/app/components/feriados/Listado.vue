	...
	<div class="main">
		<div class="content-box">
			<div class="content-box-header">
				<div class="box-title">Administrar calendario laboral</div>
				<div class="box-description">Listado calendario laboral</div>
				<div class="box-description"><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal' }">Módulos</router-link>><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal-nomina' }"> Módulo nomina</router-link>> Calendario laboral</div>
			</div>
			<div class="row">
				<div class="col-md-6 sm-text-center">
					<router-link :to="{ name: 'registrar-feriado' }" class="btn btn-success" tag="button">
						<i class="pe-7s-plus"></i> Registrar calendario laboral
					</router-link>
				</div>
			...
			<div class="table-responsive mt-3">
				<table class="table table-striped table-bordered">
					...
					<tbody>
						<tr :key="feriado.id_feriado" v-for="feriado in feriados">
							<td>{{feriado.tipo === 1 ? 'Calendario' : feriado.tipo === 2 ? 'Evento' : 'Feriado'}}</td>
							<td>{{ feriado.descripcion }}</td>
							<td>{{ feriado.fecha }}</td>
							<td class="text-center">
							 <div v-if="feriado.estado">
							...
							<td class="text-center">
								<router-link :to="{ name: 'actualizar-feriado', params: { id_feriado: feriado.id_feriado } }" tag="a"><i class="icon-pencil"></i></router-link>
							</td>
						</tr>
						<tr v-if="!feriados.length">
							<td class="text-center" colspan="5">Sin datos</td>
						</tr>
					</tbody>
				</table>
			</div>
			<pagination :items="feriados" :limit="filter.limit" :limitOptions="filter.limitOptions" :page="filter.page" :total="total" @changeLimit="changeLimit" @changePage="changePage"></pagination>
	...
	export default {
		...
		methods: {
			obtener() {
				var self = this
				self.loading = true;
				feriado.obtener(self.filter, data => {
