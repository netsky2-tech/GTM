	...
	<div class="main">
		<div class="content-box">
			<div class="content-box-header">
				<div class="box-title">Administrar impuestos</div>
				<div class="box-description">Listado de impuestos</div>
				<div class="box-description"><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal' }">Módulos</router-link>><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal-caja' }"> Módulo caja y banco</router-link>> Impuestos</div>
			</div>
			<div class="row">
				<div class="col-md-4 sm-text-center">
					<router-link :to="{ name: 'registrar-impuesto' }" class="btn btn-success" tag="button">
						<i class="pe-7s-plus"></i> Registrar impuesto
					</router-link>
				</div>
				<div @keyup.enter="filter.page = 1;obtener();" class="col-md-8 sm-text-center form-inline justify-content-end">
			...
			<div class="table-responsive mt-3">
				<table class="table table-striped table-bordered">
					...
					<tbody>
						<tr :key="impuesto.id_impuesto" v-for="impuesto in impuestos">
							<!--<td>{{ impuesto.id_impuesto }}</td>-->
							<td>{{ impuesto.descripcion }}</td>
							<td>{{ impuesto.tasa }}</td>
							<td class="text-center">
							 <div v-if="impuesto.estado">
							...
							<td class="text-center">
								<router-link :to="{ name: 'actualizar-impuesto', params: { id_impuesto: impuesto.id_impuesto } }" tag="a"><i class="icon-pencil"></i></router-link>
							</td>
						</tr>
			...
			<pagination :items="impuestos" :limit="filter.limit" :limitOptions="filter.limitOptions" :page="filter.page" :total="total" @changeLimit="changeLimit" @changePage="changePage"></pagination>
	...
	export default {
		...
		methods: {
			obtener() {
				var self = this;
				self.loading = true;
				impuesto.obtener(self.filter, data => {
			...
			downloadItem (ext) {
				...
				if(!this.descargando){
					...
					axios.get('impuestos/reporte/'+ext, { responseType: 'blob' })
							.then(({ data }) => {
