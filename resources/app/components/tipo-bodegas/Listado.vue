	...
	<div class="main">
		<div class="content-box">
			<div class="content-box-header">
				<div class="box-title">Administrar tipo bodegas</div>
				<div class="box-description">Listado de Tipo de bodegas</div>
				<div class="box-description"><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal' }">Módulos</router-link>><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal-inventario' }"> Módulo inventario</router-link>> Tipo bodega</div>
			</div>
			<div class="row">
				<div class="col-md-6 sm-text-center">
					<router-link :to="{ name: 'registrar-tipo-bodega' }" class="btn btn-success" tag="button">
						<i class="pe-7s-plus"></i> Registrar Tipo bodega
					</router-link>
				</div>
			...
			<div class="table-responsive mt-3">
				<table class="table table-striped table-bordered">
					...
					<tbody>
						<tr :key="tipo.id_tipo_bodega" v-for="tipo in tipos">
							<td>{{ tipo.descripcion }}</td>
							<td class="text-center">
							 <div v-if="tipo.estado">
							...
							<td class="text-center">
								<router-link :to="{ name: 'actualizar-tipo-bodega', params: { id_tipo_bodega: tipo.id_tipo_bodega } }" tag="a"><i class="icon-pencil"></i></router-link>
							</td>
						</tr>
			...
			<pagination :items="tipos" :limit="filter.limit" :limitOptions="filter.limitOptions" :page="filter.page" :total="total" @changeLimit="changeLimit" @changePage="changePage"></pagination>
	...
	export default {
		...
		methods: {
			obtener() {
				var self = this;
				self.loading = true;
				tipoBodega.obtener(self.filter, data => {
			...
			downloadItem (ext) {
				...
				if(!this.descargando){
					...
					axios.get('inventario/tipo-bodega/reporte/'+ext, { responseType: 'blob' })
							.then(({ data }) => {
